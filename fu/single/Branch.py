...
class Branch( Fu ):
  ...
  def construct( s, DataType, ConfigType ):
    ...
    super( Branch, s ).construct( DataType, ConfigType )
    ...
    def comb_logic():
      if s.recv_opt.msg.ctrl == OPT_BRH:
        ...
        if s.recv_in1.msg.payload == Bits16( 0 ):
  ...
  def line_trace( s ):
    ...
    symbol1 = "?"
    winner  = "nobody"
    if s.send_out0.msg.predicate == Bits1(1):
      symbol0 = "*"
      symbol1 = " "
      winner  = " if "
    elif s.send_out1.msg.predicate == Bits1(1):
      symbol0 = " "
      symbol1 = "*"
      winner  = "else"
    return f'[{s.send_out0.msg} {symbol0}] ({winner}) [{s.send_out1.msg} {symbol1}]'
