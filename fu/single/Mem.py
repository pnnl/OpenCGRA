  ...
  def construct( s, DataType, ConfigType ):
    ...
    def comb_logic():
      if s.recv_opt.msg.config == OPT_LD:
        s.send_out.msg.predicate = s.recv_in0.msg.predicate
        s.send_out.msg.payload = s.sram[s.recv_in0.msg.payload]
      elif s.recv_opt.msg.config == OPT_STR:
        s.sram[s.recv_in0.msg.payload] = s.recv_in1.msg.payload

  def line_trace( s ):
    if s.recv_opt.msg.config == OPT_LD:
      return f'{OPT_SYMBOL_DICT[s.recv_opt.msg.config]} [{s.recv_in0.msg}] = [{s.send_out.msg}]'
    elif s.recv_opt.msg.config == OPT_STR:
